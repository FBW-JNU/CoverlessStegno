...
class ADE20KDataset(Pix2pixDataset): 
    ...
    def get_secret_img(self,opt): 
        ...
        if opt.phase == 'train':
            lines = random.sample(lines, self.dataset_size)
        elif opt.phase == 'test':
            lines = lines[:self.dataset_size] 
        secret_img_list = [i.strip() for i in lines]
        return secret_img_list
