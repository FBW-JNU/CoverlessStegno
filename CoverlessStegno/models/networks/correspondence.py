...
class NoVGGCorrespondence(BaseNetwork):
    ...
    def forward(self,
        ...
        if self.opt.warp_patch:
            ref = F.unfold(ref_img, self.opt.down, stride=self.opt.down)  # self.opt.down =4
        else:
        ...
        if self.opt.warp_cycle_w > 0:
            ...
            if self.opt.warp_patch: 
                ...
                if self.opt.two_cycle:
                    ...
                    warp_i2r = torch.matmul(f_div_C_v, real_img).permute(0, 2, 1).contiguous() 
